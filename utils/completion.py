...
The arguments can be fetched by using the musdl
command.
"""
...
DEFAULT_TEMPLATE = "musdl-{}-completion.in"
FINAL_NAME = "musdl.{}"


def build_files():
    """Build the files"""
    environ["PYTHONPATH"] = "./musdl"
    process = Popen(
    ...
    for shell in SHELLS_SUPPORTED:
        ...
        # Update the musdl_opts with the opts
        template = template.replace("{{musdl_opts}}", opts)
